...
解释：
6472K->140K(9216K):因为分配allocation4时新生代空间不足，触发GC，新生代占用量减少。
6472K->6284K(19456K)：总堆占用量不变。因为allocation1~3对象依然存活，因此新生代剩余空间不足以分配给4，导致Minor GC的触发，allocation1~3通过分配担保机制提前转移到老年代。
def new generation   total 9216K：可能会有疑惑，新生代不是10M吗，怎么成了9M。因为新生代采用的复制算法回收空间，有一个Survivor（1M）被用来作为回收对象使用，因此这里显示的可用新生代总空间。
tenured generation   total 10240K：老年代6144/10240。
*/
